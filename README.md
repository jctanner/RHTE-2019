...
A) To see how the sausage will be made in the near future ...
B) We're going to create a "collection" and put something interesting into it (a module and a role) ...
C) We're going to take on the persona of a customer/user and install the collection and put it to use ...
...
2. If you've ever used the ansible-galaxy CLI to "init" a role, this next step should be very similar ...
	...
	What's this "collection" subcommand all about you ask? Well, we had a choice to make:
		A) productize an experimental project called "mazer" in a short window of time
		B) break backwards compat with ansible-galaxy install to support collections
		C) merge mazer's functionality into ansible-galaxy as a new subcommand by 2.9.0 GA
